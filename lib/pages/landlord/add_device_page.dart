...
import 'web_qr_scanner.dart' if (dart.library.html) 'web_qr_scanner.dart';
...
class _AddDevicePageState extends State<AddDevicePage> {
  ...
  Future<void> _handleSubmit() async {
    ...
    final newDevice = Device(
      ...
      pricePerUnit: _selectedType == DeviceType.electricMeter ? (double.tryParse(_electricPrice) ?? 0.5)
      ...
      SizedBox(width: double.infinity, height: 50, child: ElevatedButton(
        ...
        child: _isSubmitting
            ...
            : Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('\u786e\u8ba4\u6dfb\u52a0 ', style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w600)),
              ])),
      ),
      const SizedBox(height: 12),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE0E0E0))),
        child: Column(children: [
          Row(children: [
            Icon(_batteryLevel > 50 ? Icons.battery_std : _batteryLevel > 20 ? Icons.battery_4_bar : Icons.battery_alert,
                color: _batteryLevel > 50 ? const Color(0xFF10B981) : _batteryLevel > 20 ? const Color(0xFFF59E0B) : const Color(0xFFEF4444), size: 28),
            const SizedBox(width: 12),
            Expanded(child: Text('当前电量：$_batteryLevel%', style: const TextStyle(fontSize: 14))),
            Text(_batteryLevel > 50 ? '电量充足' : _batteryLevel > 20 ? '建议更换' : '立即更换',
                style: TextStyle(fontSize: 12, color: _batteryLevel > 50 ? const Color(0xFF10B981) : _batteryLevel > 20 ? const Color(0xFFF59E0B) : const Color(0xFFEF4444))),
          ]),
          const SizedBox(height: 10),
          SliderTheme(data: SliderThemeData(
              activeTrackColor: _batteryLevel > 50 ? const Color(0xFF10B981) : _batteryLevel > 20 ? const Color(0xFFF59E0B) : const Color(0xFFEF4444),
              thumbColor: _batteryLevel > 50 ? const Color(0xFF10B981) : _batteryLevel > 20 ? const Color(0xFFF59E0B) : const Color(0xFFEF4444)),
...
class _RoomSelector extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (rooms.isEmpty) return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE0E0E0))),
...
class _QRScannerSheetState extends State<_QRScannerSheet> {
  ...
  @override
  Widget build(BuildContext context) {
    // Web platform: use html5-qrcode JS library for reliable scanning
    if (kIsWeb) {
      return WebQrScanner(
    ...
    // Mobile platform: use mobile_scanner package
    return Container(
      color: Colors.black,
