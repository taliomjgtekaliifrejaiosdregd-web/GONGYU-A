# -*- coding: utf-8 -*-
# Write the complete add_device_page.dart with QR scanning functionality
...
class _AddDevicePageState extends State<AddDevicePage> {
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_selectedRoomId == null) { _showError('请先选择关联房源'); return; }
    ...
    final newDevice = Device(
      ...
      pricePerUnit: _selectedType == DeviceType.electricMeter ? (double.tryParse(_electricPrice) ?? 0.5) : _selectedType == DeviceType.waterMeter ? (double.tryParse(_waterPrice) ?? 3.5) : 0,
    );
    ...
    return SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFF59E0B).withOpacity(0.08), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFF59E0B).withOpacity(0.2))),
        child: Row(children: [
          ...
          const SizedBox(width: 10),
          Expanded(child: Text('首次添加建议逐台添加，方便调试。如设备较多，可批量导入。', style: TextStyle(fontSize: 12, color: Colors.orange.shade800))),
        ])),
    ]));
  }
  ...
  Widget _buildForm() {
    return SingleChildScrollView(padding: const EdgeInsets.all(16), child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: _typeColor.withOpacity(0.08), borderRadius: BorderRadius.circular(12)),
        child: Row(children: [
          Container(width: 44, height: 44, decoration: BoxDecoration(color: _typeColor.withOpacity(0.15), borderRadius: BorderRadius.circular(12)),
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(6)), child: Text(subtitle, style: TextStyle(fontSize: 10, color: color))),
          ]),
  ...
  Widget build(BuildContext context) {
    if (rooms.isEmpty) {
      return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE0E0E0))),
        child: Column(children: [
          Icon(Icons.home_work_outlined, color: Colors.grey.shade400, size: 40),
          const SizedBox(height: 8),
          Text('暂无可用房源', style: TextStyle(color: Colors.grey.shade500)),
          const SizedBox(height: 4),
          Text('请先在「房源管理」中添加房源', style: TextStyle(fontSize: 12, color: Colors.grey.shade400)),
        ]));
    }
        ...
        Wrap(spacing: 8, runSpacing: 8, children: rooms.map((room) {
          ...
          return GestureDetector(onTap: () => onChanged(room['id']!, room['title']!),
            child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8), decoration: BoxDecoration(color: isSelected ? AppColors.primary.withOpacity(0.1) : const Color(0xFFF5F5F5), borderRadius: BorderRadius.circular(20), border: Border.all(color: isSelected ? AppColors.primary : const Color(0xFFE0E0E0))),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                if (isSelected) ...[const Icon(Icons.check, size: 14, color: AppColors.primary), const SizedBox(width: 4)],
                Text(room['title']!, style: TextStyle(fontSize: 12, color: isSelected ? AppColors.primary : Colors.grey.shade700, fontWeight: isSelected ? FontWeight.w600 : FontWeight.normal)),
              ])));
        }).toList()),
        if (selectedId != null) ...[
          const SizedBox(height: 10),
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: const Color(0xFF10B981).withOpacity(0.08), borderRadius: BorderRadius.circular(8)),
            child: Row(children: [const Icon(Icons.check_circle, size: 14, color: Color(0xFF10B981)), const SizedBox(width: 6), Text('已选择：${rooms.firstWhere((r) => r['id'] == selectedId)['title']}', style: const TextStyle(fontSize: 12, color: Color(0xFF10B981)))])),
        ],
      ]));
  }
}
